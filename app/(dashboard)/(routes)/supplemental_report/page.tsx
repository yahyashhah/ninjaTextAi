...
const SupplementalReport = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const response = await axios.post("/api/supplemental_report", values);
  ...
  return (
    <div className="flex flex-col " style={{ minHeight: "calc(100vh - 90px)" }}>
      ...
      <div className="px-4 lg:px-8 flex-1">
        <div className="space-y-4 mt-4">
          ...
          {message.length === 0 && !isLoading && (
            <Empty label="Let's Generate Supplemental Report!" />
          )}
      ...
      <div id="chat-system" className="mt-auto px-4 lg:px-8 relative">
        <Form {...form}>
          <form
